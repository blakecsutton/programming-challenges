...
def check_median_tree():
  ...
  sample_size = 10
  tree = kdtree.KDTree(data, 'median', sample_size)
  print("Tree created. LOL bottleneck!")
  
def partitions_to_file(tree, 
                       target, 
                       min_point, 
                       max_point, 
                       output_filename="kdtree_parts.out"):
  """ To visualize the output generated by this function, do:
      cat kdtree_parts.out | xgraph -P -bg White -tk
      
      Note that min_point and max_point are the lowest and highest points possible in the
      input dataset, respectively, e.g. [0, 0] and [100, 100]
  """
...
def stress_test():
  """ This is a function for an idea of the order of magnitude of running time
      on the maximum inputs. """
  ...
  test_points = sampling.sample_square(origin, size, queries)
...
def check_tree():
  ...
  test_points = sampling.sample_square(origin, side_length, queries)                                                                                        
